var canvas = document.querySelector('#canvas'),
   ...
   setTimeout(function(){
    ...
    canvas.onclick=function(e){
      clickTime++
      splitCount = clickTime*4 //截取位置
      console.log(clickTime)
      if(splitCount > content.length){
      ...
      isInput = true;
      document.querySelector('#txt').value=content.slice(lastsplitCount,splitCount)  //设置字体
      lastsplitCount = splitCount
      init();
    }
      // var a='snaidjsansadjaskjndk'
    ...
    function init(){
        ...
        setFontSize(s);
        if(i<5){  //此时为一个字的时候移动端居中
          halfHeight = canvas.height / 2 - fontSize / 2;
       }else{
         halfHeight = canvas.height / 2 - fontSize / 5;
       }
